...
/**
 * ACORCLOUD GREEN-SYNC: Public Integrity Verifier
 *
 * Identical logic to verifyPeriodIntegrity, but requires NO authentication.
 * Intended for external auditors accessing /integrity/verify/:period_id
 * or the audit.acorgreensync.com subdomain.
 *
 * Uses service-role only — no user context, no sensitive data exposure.
 * Returns only the fields an auditor needs to trust the seal.
 */
...
Deno.serve(async (req) => {
  try {
    ...
    // 1. Fetch the reporting period (service role — no auth needed)
    let period;
    try {
      period = await base44.asServiceRole.entities.ReportingPeriod.get(period_id);
    } catch {
      return Response.json({ error: 'Reporting period not found' }, { status: 404 });
    }
    ...
    // 2. Re-fetch ALL transactions and audit logs in the period range
    ...
    // 5. Discrepancy diff if tampered
    ...
    return Response.json({
      ...
      hash_version: period.hash_version || HASH_VERSION,
      total_kg_co2e: period.total_kg_co2e,
    ...
    console.error('[publicVerifyIntegrity] Error:', error);
